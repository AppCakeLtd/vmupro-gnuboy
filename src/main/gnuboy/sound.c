...
void gb_sound_emulate(void) {
  if (!snd.rate || snd.cycles < snd.rate) return;
  ...
  for (; snd.cycles >= snd.rate; snd.cycles -= snd.rate) {
    ...
    if (S1.on) {
      ...
      if ((R_NR14 & 64) && ((S1.cnt += snd.rate) >= S1.len)) S1.on = 0;
    ...
    if (S2.on) {
      ...
      if ((R_NR24 & 64) && ((S2.cnt += snd.rate) >= S2.len)) S2.on = 0;
    ...
    if (S3.on) {
      ...
      if ((R_NR34 & 64) && ((S3.cnt += snd.rate) >= S3.len)) S3.on = 0;
    ...
    if (S4.on) {
      ...
      if ((R_NR44 & 64) && ((S4.cnt += snd.rate) >= S4.len)) S4.on = 0;
    ...
    if (output_buf >= output_end) {
      if (host.audio.callback) (host.audio.callback)(host.audio.buffer, output_buf - host.audio.buffer);
...
void gb_sound_write(byte r, byte b) {
  ...
  if (snd.cycles >= snd.rate) gb_sound_emulate();
  ...
  switch (r) {
    ...
    case RI_NR34:
      ...
      if (b & 0x80)  // Trigger
      {
        ...
        S3.on  = R_NR30 >> 7;
        if (!S3.on) return;
        for (int i = 0; i < 16; i++) REG(i + 0x30) = 0x13 ^ REG(i + 0x31);
      }
      break;
