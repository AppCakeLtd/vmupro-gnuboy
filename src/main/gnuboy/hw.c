...
static void rtc_latch(byte b) {
  if ((gbcart.rtc.latch ^ b) & b & 1) {
    gbcart.rtc.regs[0] = gbcart.rtc.s;
    gbcart.rtc.regs[1] = gbcart.rtc.m;
    gbcart.rtc.regs[2] = gbcart.rtc.h;
    gbcart.rtc.regs[3] = gbcart.rtc.d;
    gbcart.rtc.regs[4] = gbcart.rtc.flags;
  }
  gbcart.rtc.latch = b & 1;
}

static void rtc_write(byte b) {
  switch (gbcart.rtc.sel & 0xf) {
    case 0x8:  // Seconds
      gbcart.rtc.regs[0] = b;
      gbcart.rtc.s       = b % 60;
      break;
    ...
    case 0xB:  // Days (lower 8 bits)
      gbcart.rtc.regs[3] = b;
      gbcart.rtc.d       = ((gbcart.rtc.d & 0x100) | b) % 365;
      break;
    case 0xC:  // Flags (days upper 1 bit, carry, stop)
      gbcart.rtc.regs[4] = b;
      gbcart.rtc.flags   = b;
      gbcart.rtc.d       = ((gbcart.rtc.d & 0xff) | ((b & 1) << 9)) % 365;
      break;
  }
  gbcart.rtc.dirty = 1;
}
...
static void rtc_tick() {
  if ((gbcart.rtc.flags & 0x40)) return;  // rtc stop

  if (++gbcart.rtc.ticks >= 60) {
    if (++gbcart.rtc.s >= 60) {
      if (++gbcart.rtc.m >= 60) {
        if (++gbcart.rtc.h >= 24) {
          if (++gbcart.rtc.d >= 365) {
            gbcart.rtc.d = 0;
            gbcart.rtc.flags |= 0x80;
          }
          gbcart.rtc.h = 0;
        }
        gbcart.rtc.m = 0;
...
static void hw_dma(unsigned b) {
  unsigned a = b << 8;
  for (int i = 0; i < 160; i++, a++) hw.oam[i] = readb(a);
}
...
static void hw_hdma(byte c) {
  ...
  while (cnt--) writeb(dst++, readb(src++));
...
void gb_hw_reset(bool hard) {
  ...
  if (hard) {
    ...
    memset(gbcart.rambanks, 0xff, gbcart.ramsize * 8192);
    memset(&gbcart.rtc, 0, sizeof(gb_rtc_t));
  }
...
void gb_hw_updatemap(void) {
  int rombank = gbcart.rombank & (gbcart.romsize - 1);

  if (gbcart.rombanks[rombank] == NULL) {
    gnuboy_load_bank(rombank);
  }

  // ROM
  hw.rmap[0x0] = gbcart.rombanks[0];
  hw.rmap[0x1] = gbcart.rombanks[0];
  hw.rmap[0x2] = gbcart.rombanks[0];
  hw.rmap[0x3] = gbcart.rombanks[0];
  ...
  // Cartridge ROM
  hw.rmap[0x4] = gbcart.rombanks[rombank] - 0x4000;
  ...
  // Work RAM (GBC)
  if (IS_CGB) hw.rmap[0xD] = hw.wmap[0xD] = hw.rambanks[(R_SVBK & 0x7) ?: 1] - 0xD000;
 ...
 * change gbcart or sram banks, control special hardware, etc.
 *
...
static inline void mbc_write(unsigned a, byte b) {
  MESSAGE_DEBUG("mbc %d: gbcart bank %02X -[%04X:%02X]-> ", gbcart.mbc, gbcart.rombank, a, b);

  switch (gbcart.mbc) {
    case MBC_MBC1:
      switch (a & 0xE000) {
        case 0x0000:
          gbcart.enableram = ((b & 0x0F) == 0x0A);
          break;
        case 0x2000:
          if ((b & 0x1F) == 0) b = 0x01;
          gbcart.rombank = (gbcart.rombank & 0x60) | (b & 0x1F);
          break;
        case 0x4000:
          if (gbcart.bankmode)
            gbcart.rambank = b & 0x03;
          else
            gbcart.rombank = (gbcart.rombank & 0x1F) | ((int)(b & 3) << 5);
          break;
        case 0x6000:
          gbcart.bankmode = b & 0x1;
          break;
      }
      break;
    ...
    case MBC_MBC3:
      switch (a & 0xE000) {
        case 0x0000:
          ...
          break;
        case 0x2000:
          gbcart.rombank = b & (gbcart.romsize - 1);
          if (gbcart.rombank == 0) gbcart.rombank = 1;
          break;
        case 0x4000:
          gbcart.rtc.sel = b & 0x0f;
          gbcart.rambank = b & 0x07 & (gbcart.ramsize - 1);
          break;
        case 0x6000:
          rtc_latch(b);
          break;
    ...
    case MBC_MBC5:
      switch (a & 0x7000) {
        ...
        case 0x1000:
          ...
          break;
        case 0x2000:
          gbcart.rombank = (gbcart.rombank & 0x100) | (b);
          break;
        case 0x3000:
          gbcart.rombank = (gbcart.rombank & 0xFF) | ((int)(b & 1) << 8);
          break;
        case 0x4000:
        case 0x5000:
          gbcart.rambank = b & (gbcart.has_rumble ? 0x7 : 0xF);
    ...
    case MBC_HUC1: /* FIXME - this is all guesswork -- is it right??? */
      switch (a & 0xE000) {
        ...
        case 0x4000:
          if (gbcart.bankmode)
            gbcart.rambank = b & 0x03 & (gbcart.ramsize - 1);
          else
            gbcart.rombank = (gbcart.rombank & 0x1F) | ((int)(b & 3) << 5);
          break;
        case 0x6000:
...
void gb_hw_write(unsigned a, byte b) {
  ...
  switch (a & 0xE000) {
    case 0x0000:  // MBC control (gbcart ROM address space)
    case 0x2000:
    case 0x4000:
    case 0x6000:
      mbc_write(a, b);
    ...
    case 0xA000:  // Save RAM or RTC
      if (!gbcart.enableram) break;

      if (gbcart.rtc.sel & 8) {
        rtc_write(b);
      }
      else {
        if (gbcart.rambanks[gbcart.rambank][a & 0x1FFF] != b) {
          gbcart.rambanks[gbcart.rambank][a & 0x1FFF] = b;
          gbcart.sram_dirty |= (1 << gbcart.rambank);
        }
      }
      break;
...
byte gb_hw_read(unsigned a) {
  ...
  switch (a & 0xE000) {
    case 0x0000:  // gbcart ROM bank 0
      // BIOS is overlayed part of bank 0
      if (a < 0x900 && (R_BIOS & 1) == 0) {
        if (a < 0x100) return hw.bios[a];
        if (a >= 0x200 && IS_CGB) return hw.bios[a];
      }
      // fall through
    case 0x2000:
      return gbcart.rombanks[0][a & 0x3fff];

    case 0x4000:  // gbcart ROM
    case 0x6000:
      return gbcart.rombanks[gbcart.rombank][a & 0x3FFF];

    case 0x8000:  // Video RAM
      return hw.vbanks[R_VBK & 1][a & 0x1FFF];

    case 0xA000:  // gbcart RAM or RTC
      if (!gbcart.enableram) return 0xFF;
      if (gbcart.rtc.sel & 8) return gbcart.rtc.regs[gbcart.rtc.sel & 7];
      return gbcart.rambanks[gbcart.rambank][a & 0x1FFF];

    case 0xC000:  // System RAM
      if ((a & 0xF000) == 0xC000) return hw.rambanks[0][a & 0xFFF];
